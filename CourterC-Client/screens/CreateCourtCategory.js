    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        ...
        <View style={tw` mx-auto justify-center `}>
          <View style={tw`w-80 rounded-3xl mx-auto`}>
            {warning ? (
              <Text style={tw`text-red-500`}>
                Please select the courts sport Category, fill your Court's Price, and Upload minimum 1 image !!
              </Text>
            ) : null}
            <TextInput
            ...
            />
            <View style={tw`w-full h-12 mx-auto my-3 px-4 rounded-xl bg-white text-xl shadow-lg `}>
              <Picker
                selectedValue={data.selectedSport}
                style={tw`shadow-lg rounded-md text-xl`}
                ViewStyleProp={tw`text-xl`}
                placeholder="Selt"
                // style={{ placeholderTextColor: '#fff'}}
                onValueChange={(itemValue, itemIndex) => changeInputValue("selectedSport", itemValue)}
              >
                <Picker.Item label="Select sport" value="" />
                {categories.map((el) => {
                  return <Picker.Item label={el.name} value={el.id} key={el.id} />;
                })}
              </Picker>
            </View>
          </View>
          <View style={tw` `}>
            ...
            <View style={tw`h-16 flex flex-row justify-center flex-wrap`}>
              {data?.imgUrl.map((el, index) => (
                <Image source={{ uri: el.uri }} style={tw`w-20 h-20 mx-1 rounded-lg`} key={index} />
              ))}
          ...
          <TouchableOpacity onPress={addCourt} style={tw`bg-blue-600 h-11 mx-auto mt-10 rounded-xl`}>
            <Text style={tw`text-xl text-white w-80 text-center my-auto font-bold`}>Add Court Category</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
